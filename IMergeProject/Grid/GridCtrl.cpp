...
IMPLEMENT_DYNCREATE(CGridCtrl, CWnd)
 ...
 static bool tempIsha[100000];
	static int counterIsha;

	static bool tempIshaH[100000];
	static int counterIshaH; 
...
//}}AFX_MSG_MAP
	...
	ON_COMMAND(32796, CopyFromRightToLeft)
	ON_COMMAND(32794, CopyFromLeftToRight)
...
void CGridCtrl::DoHScroll(XLObservedData data)
{
	...
	int scrollPos = data.Action;//GetScrollPos32(SB_HORZ);
...
void CGridCtrl::DoVScroll(XLObservedData data)
{
    ...
    int scrollPos = data.Action; //GetScrollPos32(SB_VERT);
    ...
    case SB_THUMBTRACK:
        {
            //SetScrollPos32(SB_VERT, GetScrollPos32(SB_VERT, TRUE));		  scrollPos
			SetScrollPos32(SB_VERT, scrollPos);
            m_idTopLeftCell.row = -1;
            CCellID idNewTopLeft = GetTopleftNonFixedCell();
            
			int newCounter = counterIsha%4;
			tempIsha[counterIsha] = GetId();
			counterIsha++;

			if((newCounter >= 2)&& (GetId()==1))
			{
				Invalidate();
			}
			if((newCounter < 2)&& (GetId()==0))
			{
				Invalidate();
			}
            if (idNewTopLeft != idTopLeft)
...
void CGridCtrl::GoToCell(int r, int c)
{
	if((r < 0) || (r >= m_nRows))
	{
		return;
	...
	EnsureVisible(r, c);
	SetFocusCell(CCellID(r, c));
}

int CGridCtrl::GetRow(int X, int Y)
{
	CCellID id = GetCellFromPt(CPoint(X, Y));
	return id.row;
}

void CGridCtrl::SelectedPoint()
{
   int r = m_LeftClickDownCell.row;
   int c = m_LeftClickDownCell.col;
   int width ;
   
   CPoint point1;

  //OnRClick( GetPointClicked( r, c, point1) );

 // int table_Left = point;*/

}


BOOL CGridCtrl::IsRowVisible(int r)
{
	if((r < 0) || (r >= m_nRows))
	{
		return false;
...
CCellID CGridCtrl::GetVisibleTopLeftCell()
{
	return GetTopleftNonFixedCell(TRUE);
}


void CGridCtrl::SetRowBkColor(int r, COLORREF color)
{
	if((r < 0) || (r >= m_nRows))
	{
		return;
	...
	while(c < m_nCols)
	{
		SetItemBkColour(r, c, color);
		c++;
	}
	
}

void CGridCtrl::SetRowFgColor(int r, COLORREF color)
{
	if((r < 0) || (r >= m_nRows))
	{
		return;
	...
	while(c < m_nCols)
	{
		SetItemFgColour(r, c, color);
		c++;
	}
	
}

void CGridCtrl::SelectRow(int r)
{
...
// handle menu selection notifications
//void CGridCtrl::OnMenuSelect(UINT nItemID, UINT nFlags, HMENU hSysMenu);
//{
...
void CGridCtrl::OnVScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	data.Action = scrollPos;
	if(GetSubject() && XL_UTIL->IsVerticalScrollInterlocked())
	{
		GetSubject()->Notify(&data, data.EventType);
	}
	else
	{
...
void CGridCtrl::OnHScroll(UINT nSBCode, UINT /*nPos*/, CScrollBar* /*pScrollBar*/)
{
	...
	if((SB_THUMBTRACK == nSBCode) || (SB_THUMBPOSITION == nSBCode))
	{
		scrollPos = GetScrollPos32(SB_HORZ, TRUE);
	}
	else
	{
...
void CGridCtrl::CopyFromRightToLeft()
{
	...
	if(subject)
	{
		subject->OnCopyFromRightToLeft(cid.GetMinRow(), cid.GetMaxRow(), cid.GetMinCol(), cid.GetMaxCol());
	}
	ResetSelectedRange();
}
...
void CGridCtrl::CopyFromLeftToRight()
{
	...
	if(subject)
	{
		subject->OnCopyFromLeftToRight(cid.GetMinRow(), cid.GetMaxRow(), cid.GetMinCol(), cid.GetMaxCol());
	}
	ResetSelectedRange();
}
...
void CGridCtrl::OnRButtonUp(UINT nFlags, CPoint point)
{
    ...
    else
    {
		...
		WINDOWINFO info;
		this->GetWindowInfo((PWINDOWINFO)&info);
		
		point.y += info.rcClient.top;
		point.x += info.rcClient.left;
		if(m_id == 0)
		{			
			MainMenu.AppendMenu(MF_STRING, 32794, _T("Copy to Right"));
		}
		else
		{
			MainMenu.AppendMenu(MF_STRING, 32796, _T("Copy to Left")); 			
		}
		MainMenu.TrackPopupMenu(TPM_RIGHTBUTTON | TPM_LEFTALIGN | TPM_TOPALIGN, point.x, point.y, this);

		m_LeftClickDownCell = FocusCell;
...
void CGridCtrl::Clicked(CCellID id)
{
	//cRow = m_LeftClickDownCell.row;
	//cCol = m_LeftClickDownCell.col;
}